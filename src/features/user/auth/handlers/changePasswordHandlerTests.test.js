...
import { generateToken, hashPassword, isPasswordCorrect } from "../../../common/utils.js";
...
describe("POST /change-password", () => {
  ...
  it("should return error if oldPassword is incorrect", async () => {
    const res = await request(app)
      .post("/user/auth/change-password")
      .set("Authorization", `Bearer ${token}`)
      .send({ data: { oldPassword: "WrongPass123!", newPassword: "NewPassword123!" } });
    expect(res.statusCode).toBeGreaterThanOrEqual(400);
  });
  ...
  it("should return error if newPassword is same as oldPassword", async () => {
    const res = await request(app)
      .post("/user/auth/change-password")
      .set("Authorization", `Bearer ${token}`)
      .send({ data: { oldPassword: "OldPassword123!", newPassword: "OldPassword123!" } });
    expect(res.statusCode).toBeGreaterThanOrEqual(400);
  });
  ...
  it("should change password successfully", async () => {
    const res = await request(app)
      .post("/user/auth/change-password")
      .set("Authorization", `Bearer ${token}`)
      .send({ data: { oldPassword: "OldPassword123!", newPassword: "NewPassword123!" } });
    expect(res.statusCode).toBeGreaterThanOrEqual(200);
    expect(res.statusCode).toBeLessThan(300);

    const updatedUser = await User.findById(user._id);
    const match = await isPasswordCorrect("NewPassword123!", updatedUser.password);
