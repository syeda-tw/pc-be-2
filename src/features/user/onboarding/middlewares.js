...
const messages = {
  ...
  username: {
    ...
    max:      "Username should be between 4 and 10 characters",
...
export const validateOnboardingIndividualStep2Middleware = (req, res, next) => {
  const schema = Joi.object({
    ...
    sessionDuration: Joi.number().min(30).max(300).messages({
      "number.min": "Session duration must be at least 30 minutes",
      "number.max": "Session duration cannot exceed 300 minutes"
    }),
    sessionCost: Joi.number().min(10).max(5000).messages({
      "number.min": "Session cost must be at least $10",
      "number.max": "Session cost cannot exceed $5000"
    })
  });
  const { error } = schema.validate(req.body);
  if (error) {
