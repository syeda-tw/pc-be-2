...
const messages = {
  success: {
    clientsFetched: "Successfully fetched the archived clients."
  },
  error: {
    userNotFound: "We couldn't find your account",
    fetchError: "We couldn't fetch your archived clients. Please try again."
  }
};

const getArchivedClientsService = async (userId, params = {}) => {
...
const getArchivedClientsHandler = async (req, res) => {
  try {
    const data = await getArchivedClientsService(req.id, req.query);
    res.status(200).json({
...
export default getArchivedClientsHandler;