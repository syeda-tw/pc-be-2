...
import { generateToken, hashPassword, sanitizeUserAndAppendType } from "../../../common/utils.js";
...
const messages = {
  ...
  error: {
    ...
    clientNotFound:
      "The phone number you entered is not registered with Practicare",
  },
};
...
const createClientFromInvitation = async (phone, code, password) => {
  ...
  const clientData = {
    ...
    users: invitedClient.usersWhoHaveInvited.map((userId) => ({
      user: userId,
      status: "pending",
    })),
  };
  ...
  for (const userId of invitedClient.usersWhoHaveInvited) {
    ...
    if (user) {
      user.invitedClients = user.invitedClients.filter(
        (invitedClientId) =>
          invitedClientId.toString() !== invitedClient._id.toString()
      );
      user.clients = [
        ...user.clients,
        { client: client._id, status: "pending" },
      ];
      await user.save();
    }
  }
  ...
  try {
    ...
    res
      ...
      .json({
        ...
        data: sanitizeUserAndAppendType(client, "client"),
        token,
      });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ message: error.message || messages.error.clientCreationFailed });
  }
};
