...
const messages = {
  ...
  noSuccessfulSessions: "We couldn't create any successful sessions. Please try again.",
...
const processSingleSession = async (
  ...
  console.log('Starting processSingleSession with params:', { clientId, relationshipId, startTime, endTime, cost, date });
  ...
  try {
    ...
    console.log('Client found:', { clientId: client._id, status: client.status });
    ...
    console.log('Relationship found:', { relationshipId: relationship._id, status: relationship.status });
...
const bookFirstSessionForMultipleUserInvitedHandler = async (req, res) => {
  console.log('Starting bookFirstSessionForMultipleUserInvitedHandler with request body:', req.body);
  ...
  try {
    ...
    for (const result of sessionResults) {
      if (result.success) {
        const relationship = await Relationship.findById(result.relationship._id);
        if (relationship) {
    ...
    const response = {
      message: successfulSessions === sessions.length 
